    ...
    def partial_fit(self, X_batch,cont_indices,ord_indices):
        ...
        if np.isnan(self.window[0, 0]):
            ...
            if np.isnan(mean_cont):
                ...
                # print("self.cont_indices", self.cont_indices.shape)
                # print("np.sum(self.cont_indices)", np.sum(self.cont_indices))
                # print("np.random.normal(0, 1, size=(self.window_size, np.sum(self.cont_indices)))",np.random.normal(0, 1, size=(self.window_size, np.sum(self.cont_indices))))
                # print("self.window[:, self.cont_indices]", self.window[:, self.cont_indices])
