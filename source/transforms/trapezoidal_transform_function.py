...
class TrapezoidalTransformFunction():
    ...
    def partial_fit(self, X_batch,cont_indices,ord_indices):
        ...
        # Update the window with the new data in each batch, and overwrite the original data from the beginning when the column is full
    ...
    def partial_evaluate_cont_latent(self, X_batch):
        ...
        """
        X_cont = X_batch[:,self.cont_indices]#Consecutive values in #batch
        window_cont = self.window[:,self.cont_indices]#Continuous values in window
        Z_cont = np.empty(X_cont.shape)
        Z_cont[:] = np.nan#Hidden variables with continuous values
